...
def getoutput(*args, **kwargs):
    return subprocess.getoutput(*args, **kwargs)
    # return commands.getoutput(*args, **kwargs)
...
if candidates : # at least one candidate found , we need to check if the active window is among the candidates (for cycling)
        ...
        active_window_string = getoutput("""xprop -root | grep "_NET_ACTIVE_WINDOW(WINDOW)" """)
